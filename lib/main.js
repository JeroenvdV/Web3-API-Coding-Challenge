...
tokenHolder_1.TokenHolder.init(tokenHolder_1.TokenHolder.modelAttributes, { sequelize });
class DogApp {
    constructor() {
    ...
    processData(logData) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (logData.removed === true) {
                yield this.removeTransfer(logData.transactionHash, decoded.from, decoded.to, decoded.value);
            }
            else {
                yield this.addTransfer(logData.transactionHash, decoded.from, decoded.to, decoded.value, logData.blockNumber);
            }
        });
    }
}
